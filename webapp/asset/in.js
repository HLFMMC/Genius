 ...
 ********** In **********
 ...
 change:
 1.console fix(IE6)
 2.add base path (20140109)
 3.add before/after
 4.parser support (*.txt)
 */
~function() {
    var __base = (function() {
        var scripts = document.getElementsByTagName("script");
        var myself = scripts[scripts.length - 1];
        var selfPath = document.querySelector ? myself.src : myself
                .getAttribute("src");
        return selfPath.length > 0 ? selfPath.substring(0, selfPath
                .lastIndexOf('/') + 1) : '/';
    })();
    ...
    var __load = function(url, type, charset, callback, before, after) {
        if (__loading[url]) {
            if (callback) {
                setTimeout(function() {
                    __load(url, type, charset, callback, before, after);
                }, 1);
                return;
            }
            return;
        ...
        var n, t = type ||
                pureurl.toLowerCase().substring(pureurl.lastIndexOf('.') + 1);
        ...
        if (t !== 'js') {
            ...
            xhrObj.onreadystatechange = function() {
                if (xhrObj.readyState == 4) {
                    after && after(xhrObj.responseText);
                    //var scriptElem = document.createElement("script");
                    //document.getElementsByTagName("head")[0]
                    //        .appendChild(scriptElem);
                    //scriptElem.text = conv(xhrObj.responseText);
        ...
        n.onload = n.onreadystatechange = function(a) {
        ...
        this.start = function() {
            if (typeof (o.current) != 'function' && __waterfall[o.current]) {
                __load(__waterfall[o.current].path,
                        ...
                        __waterfall[o.current].before,
                        __waterfall[o.current].after);
            } else {
                ...
                __parallel(current.rely, (function(current) {
                    return function() {
                        __load(current.path, current.type, current.charset,
                                hook, current.before, current.after);
                    };
                })(current));
            } else {
    ...
    var __contentLoaded = function(win, fn) {
        var done = false, top = true, doc = win.document, root = doc.documentElement, add = doc.addEventListener
                ? 'addEventListener' : 'attachEvent', rem = doc.addEventListener
                ? 'removeEventListener' : 'detachEvent', pre = doc.addEventListener
                ? '' : 'on',
